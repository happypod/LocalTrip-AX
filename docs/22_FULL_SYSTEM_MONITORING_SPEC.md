...
|---|---:|---|---|---|---|
| FIX-001 | P1 | 공개 쓰기 API | `/api/inquiries`, `/api/partner-applications`, `/api/premium-pr-applications`, `/api/lead-events`, `/api/lead-events/map`에 rate limit 또는 Vercel Firewall/WAF 기준 추가 | 공개 운영 시 DB 스팸, LeadEvent 폭증, 문의 테이블 오염 가능 | 동일 IP/짧은 시간 반복 요청 제한 또는 Vercel 측 방어 정책 문서화 및 적용 |
| FIX-002 | P2 | 관리자 페이지 | DB 조회가 있는 관리자 `page.tsx`에서 `requireAdminSession()`을 데이터 조회보다 먼저 호출 | 비인증 요청이 DB 조회를 유발하는 구조 제거 | `src/app/admin/**/page.tsx` 중 DB 조회 페이지가 모두 조회 전 인증 확인 |
| FIX-003 | P2 | Prisma client | `src/lib/prisma.ts`의 `log: ["query"]`를 development 환경으로 제한 | 운영 로그 비용 증가 및 쿼리 노출 리스크 감소 | production에서는 query log 비활성 |
...
| FIX-006 | P2 | 보안 헤더 | CSP 적용 설계 및 단계적 도입 | 현재 기본 보안 헤더는 있으나 script/frame/img 제어가 부족 | Naver Map, YouTube, Matterport, Vimeo 허용 목록 포함 CSP 초안 작성 후 QA |
...
| ID | 우선순위 | 추가 항목 | 설명 |
|---|---:|---|---|
| ADD-001 | P1 | 운영 Smoke Test 스크립트 | `/`, `/stays`, `/experiences`, `/programs`, `/courses`, `/events`, `/map`, `/partner/apply`, `/partner/premium-pr`, `/admin` redirect를 자동 확인 |
