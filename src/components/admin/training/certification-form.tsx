...
import { createCertification, updateCertification } from "@/app/admin/training/actions";
...
export function CertificationForm({ initialData, regions }: CertificationFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 flex flex-col gap-6 max-w-xl">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-bold text-gray-700 uppercase tracking-wider">인증항목명</label>
        <input
          type="text"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="예: 친환경 업사이클링 체험 공간 인증"
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-bold text-gray-700 uppercase tracking-wider">인증 요약 (선택)</label>
        <textarea
          value={summary}
          onChange={(e) => setSummary(e.target.value)}
          placeholder="인증 항목에 대한 간략한 핵심 정보를 입력하세요."
          rows={3}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        >
          ...
          <option value="inactive">Inactive (비공개/만료)</option>
        </select>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-50">
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? "저장 중..." : initialData ? "인증 수정 완료" : "인증 등록 완료"}
        </button>
      </div>
    </form>
  );
