...
import { Certification, PublishStatus, Region } from "@prisma/client";
import { updateCertificationStatus } from "@/app/admin/training/actions";

type CertificationWithRegion = Certification & {
  region: Region;
};

interface CertificationListProps {
  certifications: CertificationWithRegion[];
}
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-50 text-gray-600 border-b border-gray-100">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold">인증항목명</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {certifications.map((cert) => (
              <tr key={cert.id} className="hover:bg-gray-50/50 transition-colors">
                <td className="px-6 py-4 text-gray-500 font-medium">{cert.region.name}</td>
                <td className="px-6 py-4">
                  <span className="font-semibold text-gray-900">{cert.title}</span>
                </td>
                <td className="px-6 py-4 text-gray-500 max-w-[200px] truncate">
                  {cert.summary || <span className="text-gray-300">-</span>}
                </td>
                ...
                <td className="px-6 py-4">
                  <select 
                    value={cert.status} 
                    onChange={(e) => handleStatusChange(cert.id, e.target.value as PublishStatus)}
                    className={`text-xs px-2 py-1 rounded-full font-medium border outline-none cursor-pointer ${
                      cert.status === 'published' ? 'bg-green-50 text-green-700 border-green-200' :
                      cert.status === 'inactive' ? 'bg-red-50 text-red-700 border-red-200' :
                ...
                <td className="px-6 py-4 text-right">
                  <Link 
                    href={`/admin/training/certifications/${cert.id}/edit`}
